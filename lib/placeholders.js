...
/**
 * Source domains that consistently serve site logos or brand headers as their
 * og:image rather than article-specific editorial photography. Skipping the
 * image fetch for these avoids storing logos in the feed.
 *
 * Add new offenders here as they appear — the SVG/Pollinations fallback will
 * kick in automatically for any domain on this list.
 */
const LOGO_SOURCE_DOMAINS = new Set([
  'jdsupra.com',
  'lexology.com',
  'newkerala.com',
  'familylaw.co.uk',
...
function isLogoSourceDomain(url) {
  if (!url) return false;
  try {
    const host = new URL(url).hostname.toLowerCase().replace(/^www\./, '');
    return LOGO_SOURCE_DOMAINS.has(host) ||
      [...LOGO_SOURCE_DOMAINS].some(d => host.endsWith('.' + d));
  } catch {
    return false;
  }
}
...
async function resolveImage(article, opts) {
  ...
  // Skip og:image fetch for domains known to serve logos/brand headers
  const skipImageFetch = isLogoSourceDomain(articleUrl);

  const socialImage = String(article.socialimage || '').trim();
  if (!skipImageFetch && socialImage && isQualityImage(socialImage)) {
    return { url: socialImage, documented: true };
  }

  if (!skipImageFetch && articleUrl && !/news\.google\.com/i.test(articleUrl)) {
