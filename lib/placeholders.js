...
function parseInlineImageCandidates(html, pageUrl) {
  ...
  for (const tag of tags) {
    ...
    if ((width && width < 240) || (height && height < 160)) continue;
...
function scoreImage(url, title, meta = "", width = 0, height = 0) {
  ...
  if (isLikelyDefaultThumb(u)) score -= 85;
  if (isLikelyGenericScene(u)) score -= 30;
  if (/(^|\/)(author|authors|staff|writer|journalist|editor)(\/|$)/.test(u)) score -= 35;
  if (/(headshot|profile|avatar|bio-photo|portrait)/.test(u)) score -= 40;
  if (/(author|staff|writer|journalist|editor|headshot|profile|avatar)/.test(m)) score -= 30;
  ...
  if (width && height) {
    ...
    if (width < 220 || height < 220) score -= 25;
    if (ratio < 0.78 && width <= 420 && height <= 640) score -= 22;
  }
  const titleTokens = t
    .replace(/[^a-z0-9\s]/g, " ")
    .split(/\s+/)
...
async function fetchArticleImage(pageUrl, title) {
  ...
  try {
    ...
    return scored[0].score >= 10 ? scored[0].u : "";
  } catch {
    return "";
  }
}
...
async function resolveImage(item, options = {}) {
  ...
  if (isHttpUrl(direct) && directScore >= 10) {
  ...
  if (isHttpUrl(extracted) && extractedScore >= 10) {
    await upsertCachedThumb(client, {
      article_url: cacheKey,
