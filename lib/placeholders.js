const CATEGORY_STYLES = {
  fraud: "editorial illustration, cybercrime, fraud investigation, muted navy and red tones",
  political: "editorial news illustration, government, elections, media integrity, muted blue and gray tones",
  entertainment: "editorial magazine illustration, celebrity culture, media technology, warm neutral tones",
  default: "editorial news illustration, synthetic media incident, realistic reportage style",
};

function pickCategoryStyle(category) {
...
function cleanHeadline(title) {
  return String(title || "")
    .replace(/\s+[-–|]\s+\S+(\.\S+)+\s*$/i, "")
    .replace(/\s+[-–|]\s+(bbc|cnn|reuters|ap|guardian|times|post|wired|vice|news)\b.*$/i, "")
    .replace(/\s+/g, " ")
    .trim();
}

function extractSubject(title, summary = "") {
  const base = cleanHeadline(title) || String(summary || "");
  const tokens = base
    .replace(/[^a-z0-9\s'-]/gi, " ")
    .split(/\s+/)
    .filter(Boolean)
    .filter((w) => !/^(the|a|an|and|or|to|for|with|from|into|about|after|over|under|amid|says?|report|reports?)$/i.test(w));
  return tokens.slice(0, 8).join(" ") || "synthetic media incident";
}
...
function pollinationsUrl(input, categoryArg = "", summaryArg = "") {
  const title = typeof input === "string" ? input : String(input?.title || "");
  const category = typeof input === "string" ? categoryArg : String(input?.category || "");
  const summary = typeof input === "string" ? summaryArg : String(input?.summary || input?.description || "");
  const style = pickCategoryStyle(category);
  const subject = extractSubject(title, summary);
  const prompt = [
    style,
    `subject: ${subject}`,
    "photorealistic editorial composition",
    "wide angle framing",
    "no text, no logos, no watermark, no infographic, no random landscape",
  ].join(", ");
  const safe = encodeURIComponent(prompt);
  const seed = stableSeed(`${title}|${category}|${subject}`);
  return `https://image.pollinations.ai/prompt/${safe}?width=800&height=450&nologo=true&seed=${seed}`;
}
...
function isLikelyGenericScene(url) {
  const u = String(url || "").toLowerCase();
  return /(road|mountain|landscape|forest|beach|onion|shells|city-night|stock-photo|unsplash)/.test(
    u
  );
}
...
function isLikelyDefaultThumb(url) {
  ...
  return /(default[-_]?image|placeholder|fallback-graphics|top_image|og[_-]?image|social[_-]?image|fbshare|site-share|share-image|brand[-_]?image|no-image|coming-soon|\/themes\/.*og-image|\/theme\/images\/fbshare)/.test(
    u
  ) || /\/mugs\/socialmedia\d*\/|app[-_]?icons?|social[-_]?icons?/.test(u);
}
...
function scoreImage(url, title, meta = "", width = 0, height = 0) {
  ...
  if (/\/mugs\/socialmedia\d*\/|app[-_]?icons?|social[-_]?icons?/.test(u)) score -= 28;
...
async function resolveImage(item, options = {}) {
  ...
  const fallback = pollinationsUrl({
    title,
