...
function deepfakeRelevanceScore(title, summary, sourceHint = "") {
  ...
  const trustedDomains = [
    "bbc.co.uk", "bbc.com", "independent.co.uk",
    "theguardian.com", "ft.com", "foreignpolicy.com",
    "france24.com", "aljazeera.com", "dw.com",
    "npr.org", "axios.com", "wired.com",
    "nbcnews.com", "abcnews.go.com", "thehill.com",
    "restofworld.org", "theintercept.com", "propublica.org"
  ];
  const isTrusted = trustedDomains.some((d) => source.includes(d));
  const aiRiskTerms = [
    "misinformation", "manipulated", "fabricated", "impersonation",
    "synthetic", "fake video", "cloned", "hoax", "disinformation",
    "ai-generated", "ai generated", "artificial intelligence"
  ];
  const combined = `${t} ${s}`;
  const hasAiRisk = aiRiskTerms.some((term) => combined.includes(term));
  if (isTrusted && hasAiRisk) score = Math.max(score, 1);
  return score;
}
