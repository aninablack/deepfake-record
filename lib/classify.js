...
function deriveSourcePriority(sourceDomain = "") {
  ...
  if (/(factcheck|snopes|politifact|fullfact|bellingcat|dfrlab|leadstories)/.test(d)) return "factchecker";
  if (
    /(bbc|ft\.com|nytimes|guardian|reuters|apnews|afp|cnn|washingtonpost|propublica|theintercept|restofworld|theverge|techcrunch|technologyreview|cyberscoop|therecord\.media|krebsonsecurity|darkreading|404media|ftc\.gov|justice\.gov|ncsc\.gov\.uk)/.test(d)
  ) return "major_outlet";
  return "other";
}
